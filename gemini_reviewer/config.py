...
class Config:
    ...
    def get_review_prompt_template(self) -> str:
        ...
        - "reviewComment": Explique o problema de forma clara. Inicie o comentário com o símbolo adequado (❌, ⚠️ ou ✅). OBRIGATÓRIO: Logo após a explicação, exiba um bloco de código `diff` ilustrando o problema. Sempre que o trecho de código analisado permitir, você DEVE incluir as 5 linhas de código anteriores intactas (sem prefixo), a(s) linha(s) com erro em vermelho (usando o prefixo `-`) e as 3 linhas de código posteriores intactas (sem prefixo). Use espaços em branco iniciais nas linhas de contexto para alinhar visualmente com o `-`. NÃO inclua nenhum título de sugestão neste campo.
        - "suggestion": Se houver código para sugerir, inicie a string com o título e pule uma linha antes de abrir o bloco interativo do GitHub (exemplo: **Sugestão**\\n\\n```suggestion\\n código corrigido aqui \\n```). Isso garante que o título fique sempre colado acima da sugestão. Certifique-se de usar os escapes de quebra de linha (\\n). Se não houver código para sugerir, retorne null.
