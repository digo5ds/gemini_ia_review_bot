...
class Config:
    ...
    def get_review_prompt_template(self) -> str:
        ...
        - "reviewComment": Explique o problema de forma clara. Inicie o comentário com o símbolo adequado (❌, ⚠️ ou ✅). OBRIGATÓRIO: Logo após a explicação, você deve exibir o código problemático em vermelho usando um bloco de código `diff` (exemplo: ```diff\\n- código errado aqui \\n```). Em seguida, pule uma linha e adicione exatamente o título **Fix Suggestion**.
        - "suggestion": Forneça a sugestão de refatoração estritamente usando o bloco de sugestão do GitHub (exemplo: ```suggestion\\n código aqui \\n```). Isso criará o botão interativo para aceitar a mudança com um clique logo abaixo do título "Fix Suggestion". Certifique-se de usar os escapes de quebra de linha (\\n) adequadamente dentro da string JSON. Se não houver código para sugerir, retorne null.
